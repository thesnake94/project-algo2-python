...
for result in searchCard_elements:
   ...
   fuel = soup.find_all(class_="Text_Text_text Vehiculecard_Vehiculecard_characteristicsItems Text_Text_body2")[3]
   mileage = soup.find_all(class_="Text_Text_text Vehiculecard_Vehiculecard_characteristicsItems Text_Text_body2")[1]
   ...
   if result:
      ...
      mileage_int = int(mileage.text.replace(" ", "").replace("km", ""))
...
with open("bmw.csv", "w", newline="") as fd:
   ...
   writer.writerow(["Marque" ,"Modèle", "Motorisation", "Année", "Prix", "Energie", "Kilométrage", "Département"]) # Écrire les en-têtes
